...
class Architect(object):
    #这个类不是对模型结构的改变，而是对训练方法的改变。如果unrolled=False，就简单的多。
    ...
    def step(self, input_train, edge_index,target_train, mask_testOld, target_valid, eta, network_optimizer, unrolled):
        self.optimizer.zero_grad()
        if unrolled:  #如果unrolled是false，这个类里面其他的方法全都用不到，直接就反向传播优化了。
            self._backward_step_unrolled(input_train, target_train, input_valid, target_valid, eta, network_optimizer)
        else:
            self._backward_step(input_train,edge_index , mask_testOld, target_valid)
        for weights in self.model.arch_parameters():
            if not weights.requires_grad:
                weights.grad = None
        self.optimizer.step()

    def _backward_step(self, input_train, edge_index,mask_testOld, target_valid):
        loss = self.model._loss(input_train, edge_index,target_valid,mask_testOld)  #SGD方法优化
        loss.backward()
